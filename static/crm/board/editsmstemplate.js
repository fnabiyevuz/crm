...
let $smsselect = $("#smstype")
let searched_leads = []
let bayramVaBoshqalar = "Bayram va boshqalar"
...
$(document).ready(function () {
    ...
    function sms_type_change() {
        if ($smsselect.val() === bayramVaBoshqalar) {
    ...
    function appent_tbody1(element) {
        $("#table1 tbody").append(
            `<tr>
                    <td>${element.name}</td>
                    <td>${isnull(element.surname)}</td>
                    <td>${isnull(element.price)}</td>
    ...
    function save_template() {
        ...
        if (sms_type !== bayramVaBoshqalar) {
            $.ajax({
                type: "POST",
                url: '/save_sms_template/',
                headers: {
                    "X-CSRFToken": getCookie("csrftoken")
                },
                data: {
                    pk: current_template['id'],
                    name: name,
                    smstext: smstext,
                    sms_type: sms_type,
                    date: ``,
